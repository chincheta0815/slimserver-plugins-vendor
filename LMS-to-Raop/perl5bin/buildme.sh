...
function build {
    case "$1" in
        CryptX)
            build_module CryptX-0.051
            ;;

        Crypt::Ed25519)
            export PERL_CANARY_STABILITY_NOPROMPT=1
            export PERL_CANARY_STABILITY_COLOUR=0

            build_module Canary-Stability-2012
            build_module Crypt-Ed25519-1.04

            unset PERL_CANARY_STABILITY_NOPROMPT 
            unset PERL_CANARY_STABILITY_COLOUR
            ;;

        Crypt::SRP)
            build_module Crypt-SRP-0.017
            ;;

        Data::Plist)
            build_module UNIVERSAL-require-0.18
            build_module Data-Plist-0.1 "" 0
            ;;
    esac
}
